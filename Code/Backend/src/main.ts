...
app.get("/api/item/getItems", (req: Request, res: Response) => {
  let parseUrl = url.parse(req.url, true)

  ItemManager.getItems(Number(parseUrl.query['count']) || 30, parseUrl.query['asc'] == null, parseUrl.query['filter'] || "id", Number(parseUrl.query['index']) || 0).then((value) => { res.write(JSON.stringify(value)); res.end(); return 1 })
})

app.get("/api/item/addItem", (req: Request, res: Response) => {
  let parseUrl = url.parse(req.url, true)
...
app.get("/api/item/addProvider", (req: Request, res: Response) => {
  let parseUrl = url.parse(req.url, true)
...
app.get("/api/item/deleteProvider", (req: Request, res: Response) => {
  let parseUrl = url.parse(req.url, true)
...
app.get("/api/item/provideItem", (req: Request, res: Response) => {
  let parseUrl = url.parse(req.url, true)
...
app.get("/api/item/addAction", (req: Request, res: Response) => {
  let parseUrl = url.parse(req.url, true)
